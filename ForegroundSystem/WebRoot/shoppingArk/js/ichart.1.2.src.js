...
(function(window) {
	...
	var iChart_ = (function(window) {
		...
		/**
		 * @method anole,make color dcabineter or lighter
		 * @param {Boolean} d true:dcabinet,false:light
...
$.Painter = $.extend($.Element, {
	...
	configure : function() {
		...
		this.set({
			...
			/**
			 * @cfg {float} Specifies the factor make color dcabinet or light for this element,relative to background-color,the bigger the value you set,the larger the color changed.scope{0.01 - 0.5}.(defaults to '0.15')
			 */
			color_factor : 0.15,
			/**
			 * @inner {String} ('2d','3d')
	...
	applyGradient:function(x,y,w,h){
		...
		if(_.get('gradient')&&_.get('f_color')){
			_.push('f_color', _.T.gradient(x||_.x||0,y||_.y||0,w||_.get(_.W),h||_.get(_.H),[_.get('dcabinet_color'), _.get('light_color')],_.get('gradient_mode')));
	...
	doConfig : function() {
		...
		_.push("dcabinet_color", $.dcabinet(bg, f*0.8,g));
	...
	Cans.prototype = {
		...
		sector3D : function() {
			var x0, y0,sPaint = function(x, y, a, b, s, e, ccw, h, c) {
				...
				angle = s;
				this.fillStyle($.dcabinet(c)).moveTo(x + a * cos(s), y + (b * sin(s))).beginPath();
				while (angle <= e) {
					Lo.call(this, angle);
					angle = angle + inc;
				}
		...
		cube : function(x, y, xv, yv, width, height, zdeep, bg, b, bw, bc, sw) {
			 ...
			 */
			this.polygon($.dcabinet(bg), b, bw, bc, false, false, [{x:x, y:y}, {x:x1, y:y1}, {x:x1 + width, y:y1}, {x:x + width, y:y}]);
			this.polygon(bg, b, bw, bc, false, false, [{x:x, y:y}, {x:x, y:y + height}, {x:x + width, y:y + height}, {x:x + width,y: y}]);
			this.polygon($.dcabinet(bg), b, bw, bc, false, false, [{x:x + width, y:y}, {x:x1 + width, y:y1}, {x:x1 + width, y:y1 + height}, {x:x + width, y:y + height}]);
			return this;
		},
...
$.Coordinate2D = $.extend($.Component, {
	configure : function() {
		...
		this.set({
			...
			striped_direction : 'v',
			/**
			 * @cfg {float(0.01 - 0.5)} Specifies the factor make color dcabinet striped,relative to background-color,the bigger the value you set,the larger the color changed.(defaults to '0.01')
			 */
			striped_factor : 0.01,
			/**
	...
	doDraw : function(_) {
		...
		if (_.get('striped')) {
			var x, y, f = false, axis = _.get('axis.width'), c = $.dcabinet(_.get('background_color'), _.get('striped_factor'),0);
		}
	...
	doConfig : function() {
		...
		if (_.get('gradient') && $.isString(_.get('f_color'))) {
			_.push('f_color', _.T.avgLinearGradient(_.x, _.y, _.x, _.y + _.height, [_.get('dcabinet_color'), _.get('light_color')]));
		}
		...
		var _ = this._(),
			...
			if (_.get('gradient')) {
				if ($.isString(ws[0].color)) {
					ws[0].color = _.T.avgLinearGradient(_.x, _.y + h, _.x + w, _.y + h, [$.dcabinet(ws[0].color,f/2+0.06),$.dcabinet(ws[0].color,f/2+0.06)]);
				}
				if ($.isString(ws[1].color)) {
					ws[1].color = _.T.avgLinearGradient(_.x + offx, _.y - offy, _.x + offx, _.y + h - offy, [$.dcabinet(ws[1].color,f),$.light(ws[1].color,f)]);
				}
				if ($.isString(ws[2].color)) {
					ws[2].color = _.T.avgLinearGradient(_.x, _.y, _.x, _.y + h, [$.light(ws[2].color,f/3),$.dcabinet(ws[2].color,f)]);
				}
				_.get('bottom_style')[5].color = _.T.avgLinearGradient(_.x, _.y + h, _.x, _.y + h + _.get('pedestal_height'), [$.light(ws[3].color,f/2+0.06),$.dcabinet(ws[3].color,f/2,0)]);
			}
		...
		lay =function(C,g,z,f){
			...
			if (C &&(Q ==0 || Q ==3) || (!C && (Q ==2 || Q ==1))) {
				layer.push({
					...
					a : f.a,
					b : f.b,
					color : $.dcabinet(f.get('background_color')),
					h : f.h,
					F : f
				});
			}
