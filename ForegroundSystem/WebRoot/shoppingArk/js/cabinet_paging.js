	...
	function paginationNick(opt) {
		...
		function goPage(btn) {
			...
			arr.forEach(function(v) {
				i++;
				str += "<tr><td>" + v.cabinet + "</td><td>" + v.loker_name + "</td><td>" + v.loker_teams + "</td><td>" + v.locker_status + "</td><td>" + v.loker_site + "</td><td>" + v.loker_coordinate + "</td><td>" + v.locker_iccid + "</td><td>" + v.activation_date + "</td><td>" + v.expiry_date + "</td><td>" + v.package_type + "</td><td>" + v.pay_way + "</td><td><button class='btn" + i + "' data-toggle='modal' data-target='.modification'>修改</button>&nbsp;&nbsp;<button>删除</button></td></tr>";
			});
		...
		function createOtherBtn() {
			$('.' + pager.paginationBox).html('<div class="' + pager.btnBox + '"><button data-page="first" class="first-btn button">首页</button><button data-page="prev" class="prev-btn button">上一页</button><span class="' + pager.numBtnBox + '"></span><button data-page="next" class="next-btn button">下一页</button><input type="text" placeholder="请输入页码" class="' + pager.ipt + '"><button class="button ' + pager.goBtn + '">跳转</button><button data-page="last" class="last-btn button">尾页</button></div>');
			...
			if (pager.maxCount > pager.numBtnCount * 2) { //若最大页码数大于等于固定数字按钮总数（pager.numBtnCount*2+1）时
				...
				if (page - pager.numBtnCount > -1) { //此页左边有pager.numBtnCount页 page页码从0开始
					for (var m = pager.numBtnCount; m > 0; m--) {
						str += '<button class="button" data-page="' + (page - m) + '">' + (page - m + 1) + '</button>';
					}
				} else {
					for (var k = 0; k < page; k++) {
						str += '<button class="button"  data-page="' + k + '">' + (k + 1) + '</button>';
					}
				}
				...
				if (pager.maxCount - page > 3) { //此页右边有pager.numBtnCount页 page页码从0开始
					for (var j = 1; j < pager.numBtnCount + 1; j++) {
						str += '<button  class="button"  data-page="' + (page + j) + '">' + (page + j + 1) + '</button>';
					}
				} else {
					for (var i = page + 1; i < pager.maxCount; i++) {
						str += '<button  class="button"  data-page="' + i + '">' + (i + 1) + '</button>';
					}
				}
				...
				if (str.match(/<\/button>/ig).length < pager.numBtnCount * 2 + 1) {
					...
					if (page < pager.numBtnCount) { //此页左边页码少于固定按钮数时
						for (var n = 0; n < page; n++) { //此页左边
							str += '<button  class="button"  data-page="' + n + '">' + (n + 1) + '</button>';
						}
						str += '<button  class="button"  data-page="' + page + '" class="' + pager.currentBtn + '" disabled="disabled">' + (page + 1) + '</button>'; //此页
						for (var x = 1; x < pager.numBtnCount * 2 + 1 - page; x++) { //此页右边
							str += '<button  class="button"  data-page="' + (page + x) + '">' + (page + x + 1) + '</button>';
						}
					}
					if (pager.maxCount - page - 1 < pager.numBtnCount) {
						for (var y = pager.numBtnCount * 2 - (pager.maxCount - page - 1); y > 0; y--) { //此页左边
							str += '<button  class="button"  data-page="' + (page - y) + '">' + (page - y + 1) + '</button>';
						}
						str += '<button data-page="' + page + '" class="' + pager.currentBtn + '" disabled="disabled">' + (page + 1) + '</button>'; //此页
						for (var z = page + 1; z < pager.maxCount; z++) { //此页右边
							str += '<button class="button"  data-page="' + z + '">' + (z + 1) + '</button>';
						}
					}
				}
			} else {
